...
//
// It NEVER blocks a page. bin/genkan-health takes a couple of seconds because
...
const ROOT = process.env.GENKAN_ROOT || join(HERE, "..");
const HEALTH_BIN = join(ROOT, "bin", "genkan-health");
const CACHE_FILE = process.env.GENKAN_HEALTH_FILE || "/var/lib/genkan/health.json";
...
let VERSION = process.env.GENKAN_VERSION || "";
if (!VERSION) { try { VERSION = readFileSync(join(ROOT, "VERSION"), "utf8").trim(); } catch { VERSION = ""; } }

let cache = { at: 0, data: null, running: false };

function readCacheFile() {
  // bin/genkan-health leaves its last answer here when it can write to
  // /var/lib/genkan, which is the normal case on a deployed box: the upgrade
...
function refresh() {
  ...
  cache.running = true;
  // --json --quiet is read-only: see the header of bin/genkan-health. A
...
export function versionFooter() {
  ...
  const detail = dot === "bad"
    ? `<div class="vfix">Run <code>genkan-health</code> in a terminal for what to do. If this started after an update: <code>sudo genkan-rollback to previous</code>.</div>` : "";
  return `<style>
