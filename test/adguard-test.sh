...
psql "INSERT INTO devices(mac,label,child_id,reserved_ip,kind,category,is_active,last_seen)
      VALUES('fe:ed:00:00:00:01','adgtest-kid1',(SELECT id FROM children WHERE name='$KID1'),'192.168.60.101','phone','personal',true,now())
      ON CONFLICT (mac) DO UPDATE SET child_id=(SELECT id FROM children WHERE name='$KID1'), reserved_ip='192.168.60.101', is_active=true" >/dev/null
psql "INSERT INTO devices(mac,label,child_id,reserved_ip,kind,category,is_active,last_seen)
      VALUES('fe:ed:00:00:00:03','adgtest-kid3',(SELECT id FROM children WHERE name='$KID3'),'192.168.60.121','phone','personal',true,now())
      ON CONFLICT (mac) DO UPDATE SET child_id=(SELECT id FROM children WHERE name='$KID3'), reserved_ip='192.168.60.121', is_active=true" >/dev/null
...
want fortnite.com    192.168.60.101 RewriteRule           "11 gaming -> portal"
want googlevideo.com 192.168.60.101 RewriteRule           "11 video -> portal"
want spotify.com     192.168.60.101 NotFilteredNotFound   "11 audio stays up"
want fortnite.com    192.168.60.121 NotFilteredNotFound   "16 gaming allowed"
want 1737.org.nz     192.168.60.101 NotFilteredWhiteList  "11 helpline wins"
want khanacademy.org 192.168.60.101 NotFilteredWhiteList  "11 schoolwork wins"
want pornhub.com     192.168.60.101 FilteredBlackList     "11 adult blocked"
want pornhub.com     192.168.60.121 FilteredBlackList     "16 adult blocked"

# Unblock and confirm it clears.
psql "UPDATE category_state SET blocked=false WHERE set_by='adgtest'" >/dev/null
"$KN/kidnet-adguard" apply >/dev/null; sleep 2
want fortnite.com 192.168.60.101 NotFilteredNotFound "11 gaming restored after unblock"
