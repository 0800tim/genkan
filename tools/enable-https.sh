#!/usr/bin/env bash
# genkan:summary=Put the dashboard behind real HTTPS using a Tailscale certificate.
#
...
CERTDIR="${CERTDIR:-/var/lib/genkan/tls}"
PORT="${PORT:-8899}"
...
if ! sudo tailscale cert --cert-file "$CERTDIR/genkan.crt" --key-file "$CERTDIR/genkan.key" "$NAME" 2>&1 | tail -2; then
  cat <<MSG
...
sudo chown "$USER" "$CERTDIR" "$CERTDIR"/genkan.* 2>/dev/null || true
sudo chmod 0640 "$CERTDIR/genkan.key" 2>/dev/null || true

CADDYFILE="$CERTDIR/Caddyfile"
cat > "$CADDYFILE" <<CADDY
# Terminates HTTPS for the Genkan dashboard using the tailnet certificate,
...
https://$NAME:$TLS_PORT {
	tls $CERTDIR/genkan.crt $CERTDIR/genkan.key
	reverse_proxy $DASH_ADDR
}
CADDY

mkdir -p "$HOME/.config/systemd/user"
cat > "$HOME/.config/systemd/user/genkan-dashboard-tls.service" <<UNIT
[Unit]
Description=HTTPS front end for the Genkan dashboard (tailnet certificate)
...
systemctl --user enable --now genkan-dashboard-tls.service

systemctl --user restart genkan-dashboard-tls.service 2>/dev/null || true
sleep 2
if curl -sk --max-time 10 "https://127.0.0.1:$TLS_PORT/" -o /dev/null 2>/dev/null \
  ...
  echo "WARNING: the front end did not answer. Check: journalctl --user -u genkan-dashboard-tls -n 20"
fi
