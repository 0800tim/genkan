...
-- genkan-tor-sync fetched the public relay list every day, wrote it to a file
...
-- that reject a relay matched nothing, and genkan-health reported "the Tor
...
COMMENT ON TABLE tor_nodes IS
  'Public Tor relay addresses, refreshed daily by genkan-tor-sync. The gateway reconciles the @tor_nodes nft set from this table hourly.';
...
-- and genkan-health can say "the list is current AND in force" rather than
