-- Hearth: the least-privilege role the CLI and the background workers use.
--
...
--           the Hearth tables the scripts touch, and nothing else.
...
-- Hearth shares an instance with unrelated projects, any of those projects'
-- roles could open kids_network and read the catalogue. Only the two Hearth
...
-- not something this file can do: that would mean editing an ACL Hearth does
...
GRANT SELECT ON category_domains      TO kids_agent;  -- kidnet-adguard, kidnet-catmap
GRANT SELECT ON flag_domains          TO kids_agent;  -- kidnet-alerts, kidnet-report
GRANT SELECT ON policies              TO kids_agent;  -- daily budgets by tier
GRANT SELECT ON services              TO kids_agent;  -- kidnet-servicemap/-servicemeter
GRANT SELECT ON service_domains       TO kids_agent;
GRANT SELECT ON tasks                 TO kids_agent;  -- kidnet earn, kidnet-report
GRANT SELECT ON earn_claims           TO kids_agent;  -- kidnet-report
GRANT SELECT ON device_claims         TO kids_agent;  -- kidnet claims
GRANT SELECT ON vendor_clouds         TO kids_agent;  -- kidnet-iot-policy
GRANT SELECT ON vendor_domains        TO kids_agent;
GRANT SELECT ON vendor_aliases        TO kids_agent;
GRANT SELECT ON quiz_banks            TO kids_agent;  -- kidnet-quiz, -quiz-suggest
...
GRANT SELECT, INSERT, UPDATE         ON dhcp_leases   TO kids_agent;  -- kidnet-devicescan
GRANT SELECT, INSERT                 ON dns_log       TO kids_agent;  -- kidnet-dnslog
GRANT SELECT, INSERT, UPDATE         ON alerts        TO kids_agent;  -- kidnet-alerts, -dnslog, -iot-policy
...
-- The bedtime scheduler (bin/kidnet-schedule) and its tables. schedule_state
...
-- Notifications (config/db/schema-notify.sql). bin/kidnet-notify reads the
...
-- The release log (config/db/schema-release.sql). bin/kidnet-upgrade and
-- bin/kidnet-rollback append one row per thing that happened to this box:
