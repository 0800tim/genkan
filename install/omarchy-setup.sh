#!/usr/bin/env bash
# hearth:summary=Set up a fresh Omarchy (or any Arch) box as a Hearth gateway.
#
...
set -euo pipefail
R="$(cd "$(dirname "$0")/.." && pwd)"
log(){ printf '\033[38;5;208m[hearth]\033[0m %s\n' "$*"; }
...
if command -v nmcli >/dev/null && [ -n "${KIDS_NIC_MAC:-}" ]; then
  DROP=/etc/NetworkManager/conf.d/99-hearth-kids-nic.conf
  if [ ! -f "$DROP" ]; then
...
if [ -d "$HOME/.config/omarchy" ]; then
  ...
  if [ ! -f "$HOOK" ] || ! grep -q hearth "$HOOK"; then
