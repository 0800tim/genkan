...
1. **Dedicated isolated segment, not MAC rules on the main LAN.** Modern
   ...
   blocking. By making the kids their own segment behind clawdia, we
...
3. **Real datastore + audit.** Postgres logging (devices, DNS, alerts,
   every on/off action) instead of ephemeral router state.
...
time by studying), kids-network works at the network layer (who/what/when).
